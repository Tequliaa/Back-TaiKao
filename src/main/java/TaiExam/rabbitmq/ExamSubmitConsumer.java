...
public class ExamSubmitConsumer {
    ...
    @RabbitListener(queues = ExamMessageQueueConfig.EXAM_SUBMIT_QUEUE)
        ...
        try {
            ...
            if ("remake".equals(message.getActionType()) && "超级管理员".equals(message.getUserRole())) {
                if (message.getAnswerUserId() != null) {
                    remakeExam(message.getId(), message.getAnswerUserId());
                    return;
                }
                // 可以记录错误日志
                return;
            ...
            // 检查是否已提交
            UserExam userExam = userExamService.getUserExamByUserIdAndExamId(
                message.getUserId(), message.getId());
            if (userExam !=null&&"已完成".equals(userExam.getStatus())) {
                // 可以记录已提交的情况
                return;
            }
            
            // 初始化响应记录
            initializeResponses(message.getId(), message.getUserId(), message.getIpAddress());
            
            // 处理表单数据
            if (message.getFormData() != null && !message.getFormData().isEmpty()) {
                processFormData(message.getFormData(), message.getId(),
                    message.getUserId(), message.getIpAddress(), message.isSaveAction());
            }
            
            // 处理文件上传
            if (message.getFileMap() != null && !message.getFileMap().isEmpty()) {
                handleFileUploads(message.getFileMap(), message.getId(),
                    message.getUserId(), message.getIpAddress());
            }
            
            // 完成问卷
            completeExam(message.getId(), message.getUserId(), message.isSaveAction());
            
            // 准备通知数据
            User user = userService.getUserByUserId(message.getUserId());
            int newUnfinishedCount = userExamService.getUserInfoCount(
                message.getId(), user.getDepartmentId());

            // 准备通知数据（添加 examId 到消息体中）
            Map<String, Object> pushData = new HashMap<>();
            pushData.put("examId", message.getId()); // 新增：将 examId 放入消息体
            pushData.put("unfinishedTotalRecords", newUnfinishedCount);
            pushData.put("message", "用户【" + user.getName() + "】已完成答题");

            // 发送通知消息（只需传递 Map，无需额外传递 examId）
            examMessageProducer.sendExamNotifyMessage(pushData);
    ...
    //打回问卷
    private void remakeExam(int examId, int userId){
        userExamService.updateExamStatusByExamAndUser(examId, userId, "保存未提交",
    ...
    //初始化答题情况
    private void initializeResponses(int examId, int userId, String ipAddress){
        List<Question> questions = questionService.getQuestionsByExamId(examId);
        if (!responseService.checkResponseExists(userId, examId)) {
            ...
            List<Response> existingFileResponses = responseService.selectExistingFileResponses(userId, examId);
            ...
            if(!existingFileResponses.isEmpty()){
                ...
                responseService.resetIsValidForResponsesExcludingIds(userId, examId, existingResponseIds);
            }else {
                responseService.resetIsValidForResponses(userId,examId);
            }
        }
    ...
    // 处理表单数据
    private void processFormData(Map<String, String> formData, int examId, int userId, String ipAddress, boolean isSaveAction) throws Exception {
        // //调试输出开始
        ...
        for (Map.Entry<String, String> entry : formData.entrySet()) {
            ...
            if (paramName.startsWith("question_")) {
                ...
                if ("排序".equals(question.getType())) {
                    ...
                    processSortAnswer(paramName, paramValue, examId, questionId, ipAddress, userId);
                }
                else{
                    processQuestionAnswer(paramName,paramValue, examId, questionId, ipAddress, userId, isSaveAction);
                }
            } else if (paramName.startsWith("rating_")) {
                processRatingAnswer(paramName, paramValue, examId, ipAddress, userId);
    ...
    //处理排序题
    private void processSortAnswer(String paramName, String paramValue, int examId, int questionId,
    ...
    // 处理文件上传 - 通过文件路径获取文件
    private void handleFileUploads(Map<String, String> fileInfoMap, int examId, int userId, String ipAddress) throws IOException {
    ...
    //完成问卷
    private void completeExam(int examId, int userId, boolean isSaveAction){
        if (!isSaveAction) {
            userExamService.updateExamStatusByExamAndUser(examId, userId, "已完成",
                    new Timestamp(System.currentTimeMillis()));
        }else{
            userExamService.updateExamStatusByExamAndUser(examId, userId, "保存未提交",
                    new Timestamp(System.currentTimeMillis()));
    ...
    //创建初始答案
    private Response createInitialResponse(int examId, int questionId, int userId, String ipAddress,
                                           int rowId, int columnId) {
        Response responseRecord = new Response();
        responseRecord.setExamId(examId);
    ...
    //处理单选多选和填空题
    private void processQuestionAnswer(String paramName,String paramValue, int examId, int questionId,
        ...
        if (paramName.contains("_row_")) {
            ...
            saveMatrixResponse(examId, questionId, rowOptionId, columnOptionId, ipAddress, userId, isSaveAction);
        } else {
            saveResponse(examId, questionId,paramName, paramValue, ipAddress, userId, isSaveAction);
        }
    }

    //处理评分题
    private void processRatingAnswer(String paramName, String paramValue, int examId, String ipAddress, int userId) throws Exception {
    ...
    //处理单选多选和填空
    private void saveResponse(int examId, int questionId,String paramName, String answer, String ipAddress, int userId, boolean isSaveAction) throws Exception {
        Response response = new Response();
        response.setExamId(examId);
        response.setQuestionId(questionId);
    ...
    //处理矩阵题
    private void saveMatrixResponse(int examId, int questionId, String rowOptionId, String columnOptionId,
