...
import TaiExam.context.BaseContext;
import TaiExam.model.dto.UserDTO;
import TaiExam.model.entity.*;
import TaiExam.model.vo.UserInfoVO;
...
import TaiExam.utils.HashUtils;
...
public class UserController {
    ...
    public UserController(UserService userService, DepartmentService departmentService,
                          ...
                          UserExamService userExamService, UserRoleService userRoleService) {
    ...
    public Result<Void> register(@RequestBody User user) {
        ...
        List<DepartmentExam> departmentExams =departmentExamService.getDepartmentExams(user.getDepartmentId());
        userExamService.assignExamsToUser(userId, departmentExams);
    ...
    public Result<ImportResult> importUsers(@RequestParam("file") MultipartFile file,
        ...
        try {
            ...
            //获取部门所分发过的问卷
            List<DepartmentExam> departmentExams =departmentExamService.getDepartmentExams(departmentId);
            //当用户列表跟部门问卷列表都不为空时再执行批量补发
            if(!userList.isEmpty()&&!departmentExams.isEmpty()){
                userExamService.assignExamsToUsers(userList, departmentExams);
            }
