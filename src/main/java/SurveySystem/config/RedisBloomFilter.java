...
/**
 * 基于Redis的布隆过滤器（防缓存击穿）
 * 用于快速判断surveyId是否存在，减少对不存在ID的数据库查询
 */
@Component
@Slf4j
public class RedisBloomFilter {
    ...
    // 布隆过滤器在Redis中的Key
    private static final String BLOOM_FILTER_KEY = "bloom:filter:survey:ids";
    ...
    /**
     * 初始化：项目启动时将数据库中已有的surveyId加载到布隆过滤器
     * （实际场景可异步执行，避免启动阻塞）
     */
    @PostConstruct
    public void init() {
         List<Integer> allSurveyIds = surveyMapper.getAllSurveyIds();
         log.info("allSurveyIds:{}",allSurveyIds);
         allSurveyIds.forEach(this::add);
    }
    ...
    public void add(int surveyId) {
        byte[] bytes = String.valueOf(surveyId).getBytes(StandardCharsets.UTF_8);
    ...
    /**
     * 判断surveyId是否可能存在（存在返回true，一定不存在返回false）
     */
    public boolean mightContain(int surveyId) {
        byte[] bytes = String.valueOf(surveyId).getBytes(StandardCharsets.UTF_8);
        for (int i = 0; i < numHashFunctions; i++) {
            long index = hash(bytes, i);
